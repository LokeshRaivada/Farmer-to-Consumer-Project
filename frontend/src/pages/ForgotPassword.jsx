...
const ForgotPassword = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setError(err.response?.data?.message || 'Failed to send password reset link. Please check the email address.');
        } finally {
            setLoading(false);
        }
    };
        ...
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '80vh', padding: '2rem 1rem' }}>
            ...
            >
                <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
                    ...
                    <h2 style={{ fontSize: '1.5rem', fontWeight: '800', color: 'var(--text-light)' }}>Forgot Password?</h2>
                ...
                {success ? (
                    <div style={{ textAlign: 'center' }}>
                        ...
                        <h4 style={{ fontSize: '1.1rem', fontWeight: 'bold', color: 'var(--text-light)', marginBottom: '0.5rem' }}>Reset Link Sent!</h4>
                        <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginBottom: '1.5rem', lineHeight: '1.5' }}>
                            If an account exists for <strong>{email}</strong>, a recovery email has been sent with further instructions.
                        </p>
                        ...
                        <div style={{ background: 'rgba(22, 163, 74, 0.05)', border: '1px solid rgba(22, 163, 74, 0.1)', borderRadius: '0.75rem', padding: '1rem', marginBottom: '2rem', textAlign: 'left', fontSize: '0.8rem' }}>
                            ...
                            <span style={{ color: 'var(--text-muted)', lineHeight: '1.4' }}>
                                For offline testing, you can grab the password reset link directly from the local email log file:
                    ...
                    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                        ...
                        >
                            {loading ? <Loader2 size={18} className="animate-spin" style={{ animation: 'spin 1.5s linear infinite' }} /> : 'Send Reset Link'}
                        </button>
